...
export enum HttpStatusCode {
   ...
   * to be sent; for example, a POST request). Sending a large request body to a
   * server after a request has been rejected for inappropriate headers would be
   * inefficient. To have a server check the request's headers, a client must
   * send Expect: 100-continue as a header in its initial request and receive a
   ...
   * The HTTP/1.0 spec (RFC 1945) required the client to perform a temporary
   ...
   * the address for which is provided in the response. Many HTTP clients (such
   ...
   * URI; however, future requests should still use the original URI. In
   ...
   * has not happened, and this code is not usually used. E.g. Google Developers

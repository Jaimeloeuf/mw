...
/**
 * Utility function to parse a command from the message if any, else returns
 * null.
 *
 ...
 */
export function getCommandIfAny(update: TelegramWebhookData) {
  ...
  for (const entity of update.message.entities) {
    if (entity.type === "bot_command") {
      const command = update.message.text.slice(

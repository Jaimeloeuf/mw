 ...
 */
export function dalWrapper2<T extends (...args: any) => Promise<any>>(fn: T) {
  // Extra runtime check alongside compile time check with ESLint rule
  // 'mwEslintPlugin/require-function-name-for-dalWrapper'
  if (fn.name === "") {
    throw new Error(`Functions passed to ${dalWrapper2.name} must be named`);
  }
